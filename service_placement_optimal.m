function processDataFiles()
    ...
    % Open file to write ILP results
    resultFile = fopen('gap_ilp_result.txt', 'w');
    if resultFile == -1
        error('Cannot open gap_ilp_result.txt for writing.');
    end
    ...
    for fileIndex = 1:totalFiles
        ...
        % Write dataset name to result file
        ...
        for caseIndex = 1:totalCases
            ...
            resultLine = sprintf('c%d-%d\t%d', serverCount*100 + userCount, caseIndex, round(totalCost));
            caseResults{caseIndex} = resultLine;
